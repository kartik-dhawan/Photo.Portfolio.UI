    ...
    <div className={`grid gap-4 grid-cols-1 ${colClass}`}>
      {media.map((item, i) => {
        ...
        const brand = item.brandId
          ...
          : undefined;
        const hasCaption = item.title || item.date || item.brandId;

        if (item.thumbnailUrl && item.thumbnailSize === "large") {
          return (
            <a
              key={i}
              href={item.url}
            ...
            >
              ...
              {hasCaption && (
                <div className="flex flex-col gap-0.5 px-1 pt-1">
                  {item.title && (
                    <span className="text-white text-sm font-mono text-left">
                      {item.title}
                    </span>
                  )}
        ...
        return (
          ...
          >
            ...
            <div className="ml-auto">{externalArrow}</div>
          </a>
        );
      })}
    </div>
