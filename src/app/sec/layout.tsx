...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { type: "website", siteName: user.displayName, locale: "en_US", images },
    twitter: { card: "summary_large_image", images: settings.profilePhotoUrl ? [settings.profilePhotoUrl] : undefined },
  };
}

export default async function SecLayout({ children }: { children: React.ReactNode }) {
  const user = await getUserByUsername(DEFAULT_USERNAME);
      ...
      <TenantProvider userId={user.uid} username={user.username} displayName={user.displayName} tagline={user.tagline} socials={user.socials ?? []} hasCustomDomain={hasCustomDomain}>
        <ThemeProvider themeId={user.themeId ?? "black"}>
          ...
          <div className="flex min-h-screen">
            <aside className="hidden md:flex flex-col w-64 shrink-0 px-8 pt-12 pb-8 sticky top-0 h-screen overflow-hidden border-r border-zinc-800/50" style={{ backgroundColor: "var(--sidebar)" }}>
              <Sidebar />
            </aside>
